...
class CUBDataLoader():
    ...
    def read_matdataset(self):
        path= self.datadir + 'feature_map_ResNet_101_{}.pickle'.format(self.dataset)  # hdf5改为 pickle
        print('_____')
        print(path)
        # hf = h5py.File(path, 'r')
        ...
        trainval_loc = data_dict['trainval_loc']
        test_seen_loc = data_dict['test_seen_loc']
        test_unseen_loc = data_dict['test_unseen_loc']
        att = data_dict['att']
        original_att = data_dict['original_att']
        w2v_att = data_dict['w2v_att']
        ...
        if self.is_unsupervised_attr:
            ...
            temp = att # temp = np.array(hf.get('att'))
